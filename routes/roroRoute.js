...
//CONSIGNEE
router.get('/roro', function(req, res) {
    if (req.user) {
        //get clients
        var perPage = 4;
...
//CONSIGNEE
router.get('/cargo/displaynewroro', function(req, res) {
    if (req.user) {
        //get clients
        ...
        async.parallel([
            ...
            var clientsData = results[0];
            return res.render("main/newRoro", { title: 'Oldsailor Ocean Shipping LLC || New Cargo', clientsData: clientsData, layout: false });
        });
...
router.post("/roro/new", function(req, res) {
...
//Add More Cars
router.post("/roro/:id/addmorecars", function(req, res) {
    //check if cargo exist 
    // var vin = "";
    // if (req.body.data.vin !== "") {
...
router.get("/roro/:value/fetchCargo_perclient", function(req, res) {
...
//display one cargo
router.get('/roro/:id/oneCargo', function(req, res) {
    if (req.user) {
        Cargos.findOne({ _id: req.params.id })
            .populate("Client")
            .exec(function(err, foundData) {
...
router.get('/roro/:id/editRoute', function(req, res) {
    if (req.user) {
        Cargos.findOne({ _id: req.params.id })
            .populate("Client")
            .exec(function(err, foundData) {
                //console.log(foundData);
                res.render("main/editRoro", { layout: false, foundData: foundData });
            });
...
//edit car post route
router.post("/roro/:cargo_id/edit", function(req, res) {
    Cargos.update({
...
router.post("/roro/:cargo_id/edit_personaleffect", function(req, res) {
...
//delete cargo
router.delete("/roro/:id/delete", function(req, res) {
...
//delete Car
router.post("/roro/:car_id/delete/:cargo_id", function(req, res) {
    Cargos.update({
            _id: req.params.cargo_id
